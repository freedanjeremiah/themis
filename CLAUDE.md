...
5. **No CCTP — agents trade against pre-funded HL testnet wallets.** The original Phase 2 design bridged USDC Arc↔HL every cycle via CCTP V2. Soak testing exposed too many failure modes (V2 7-param signature mismatch, Iris testnet latency, stuck mid-roundtrip). The pre-fund approach (commit `03f467a`) won: each agent wallet holds USDC on HyperEVM (chain 998) directly and trades perps with no bridge. All CCTP code (`agent-*/src/cctp.ts`, `scripts/cctp-recover.ts`, `scripts/verify-cctp-*.ts`) and env (`CCTP_*`, `MESSAGE_TRANSMITTER_*`, `USDC_ADDRESS_HL`) has been **removed**. Vault accounting on Arc still tracks economics; PnL settles back via `vault.settle`.
